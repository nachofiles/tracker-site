...
export type SyncUpdateSuccessCallback = (err: undefined, data: SyncUpdate) => void;
export type SyncUpdateErrorCallback = (err: Error, data: undefined) => void;
export type SyncUpdateCallback = SyncUpdateSuccessCallback | SyncUpdateErrorCallback;
...
export class InodeDatabase {
  ...
  async search(
    ...
    const data = await inodes.offset(offset).limit(limit).toArray();
